...
import '../../domain/use_cases/delete_image_to_text_use_case.dart';
...
class ImageToTextCubit extends Cubit<ImageToTextState> {
  ...
  final DeleteImageToTextUseCase _deleteImageToTextUseCase;

  ImageToTextCubit(
    this._getImageToTextListUseCase,
    this._updateImageToTextUseCase,
    this._deleteImageToTextUseCase,
  ...
  Future<void> deleteImageToText({
    required String documentId,
    required String uid,
  }) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
      await _deleteImageToTextUseCase(documentId);
      final items = await _getImageToTextListUseCase(uid);
      emit(state.copyWith(
        isLoading: false,
        items: items,
